...
import { frmtprvwpstat } from '../shared/preview-waktu';

describe('frmtprvwpstat', () => {
  it('16:00 + 10h20m => 02:20 tomorrow', () => {
    const now = new Date(2026, 1, 18, 16, 0, 0);
    const hasil = frmtprvwpstat({ jam: 10, menit: 20, detik: 0 }, now);
  ...
  it('10:00 + 2h => 12:00 (same day)', () => {
    ...
    const hasil = frmtprvwpstat({ jam: 2, menit: 0, detik: 0 }, now);
  ...
  it('10:00 + 49h => in 2 days hint', () => {
    const now = new Date(2026, 1, 18, 10, 0, 0);
    const hasil = frmtprvwpstat({ jam: 49, menit: 0, detik: 0 }, now);
